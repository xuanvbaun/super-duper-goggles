...
class MultiRootTests(unittest.TestCase):
    ...
    def test_standard_part_displays_spec_standard_and_grade(self):
        row = {"ares": False, "material": "钢8.8", "name": "内六角凹端螺钉",
               "spec": "M16x60", "std": "GB/T80-2007", "cat": "16"}
        self.assertEqual(craft.material_of(row), "M16x60/GB/T80-2007/钢8.8")

    def test_supplied_part_retains_full_dimensions_in_material_and_remark(self):
        row = {"ares": True, "name": "冷弯方形空心钢", "material": "0Cr25Ni20", "spec": "25x25x1.5 L=1845"}
        self.assertEqual(craft.material_of(row), "冷弯方形空心钢25x25x1.5/0Cr25Ni20")
        self.assertEqual(craft.remark_of(row), row["spec"])

    def test_missing_grade_does_not_hide_spec(self):
        self.assertEqual(craft.material_of({"spec": "2.5x□21.5"}), "")
        self.assertEqual(craft.remark_of({"spec": "2.5x□21.5"}), "2.5x□21.5")
        self.assertEqual(craft.material_of({}), "")
